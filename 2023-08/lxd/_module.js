...
function create_fragment$b(ctx) {
	...
	component_0 = new Component({
			props: {
				...
				image: {
					"alt": "batter up",
					"src": "https://ogfrjxqovwgsdzubizan.supabase.co/storage/v1/object/public/images/d6c182e2-29f7-45ec-a001-8b5fa4778f97/1689875074261491167_boxing-matches_t285.webp",
	...
	component_3 = new Component$4({
			props: {
				...
				description: "lxd vs docker",
				content: {
					"html": "<p><a href=\"https://www.stoneskull.me/2023-07/web\"><-- previously</a><br><br></p>\n<h2 id=\"lxdvsdocker\">lxd vs docker</h2>\n<h4 id=\"heywhat\"><em>hey what..</em></h4>\n<p><br></p>\n<p>i hadn't heard of lxd before..</p>\n<p>i <a href=\"https://www.stoneskull.me/2023-08/blogs\">mentioned before</a> that i got a nas. and besides being storage, it can also use its computer to run stuff. media apps like plex, data back-up solutions, and these things called containers..</p>\n<p>i used docker a few years ago.. hardly remember it. once i do remember, i'll come back to this..</p>\n<p>so yeah, i'm going to learn about both these container things at the same time.</p>",
					"markdown": "[<-- previously](https://www.stoneskull.me/2023-07/web)<br><br>\n\n## lxd vs docker\n\n#### *hey what..*\n\n<br>\n\ni hadn't heard of lxd before..\n\ni [mentioned before](https://www.stoneskull.me/2023-08/blogs) that i got a nas. and besides being storage, it can also use its computer to run stuff. media apps like plex, data back-up solutions, and these things called containers..\n\ni used docker a few years ago.. hardly remember it. once i do remember, i'll come back to this..\n\nso yeah, i'm going to learn about both these container things at the same time.\n\n"
				}
			}
		});
	...
	component_4 = new Component$5({
			props: {
				...
				image: {
					"alt": "lxd v docker",
					"url": "https://ogfrjxqovwgsdzubizan.supabase.co/storage/v1/object/public/images/d6c182e2-29f7-45ec-a001-8b5fa4778f97/th-4021702650.jpeg1692425293877"
				},
				title: "LXD",
				description: "lxd vs docker"
			}
	...
	component_5 = new Component$6({
			props: {
				...
				description: "lxd vs docker",
				content: {
					"html": "<h2>what is a container?</h2><h4><em>first things first..</em></h4><p></p><p>i guess the zeroth thing is this is linux.</p><p>i've used a vm or two, which can be considered containers, on windows before. vmware and virtualbox are the two main runners of these. a vm lets an os run inside an os. there are other ones like dosbox and scummvm that are like mini operating systems inside a container, the os made to serve the one main task. and then there is wsl.. windows subsystem for linux.. a linux box in windows.</p><p>so, a container is a box, like a desktop computer is a box. there's stuff inside the box which has everything needed to operate and run a system (and run smaller boxes). </p><p>but.. power and adapters and peripherals and cables.. other things are still needed, to do much of anything, with this box.</p><p>all these things, this system, is what we must get used to. </p><p>the insides of the box and how that works isn't very important. it's what we can do with the box, and what the box can do for us.</p><p>if we want to see what a computer is doing, we have to plug in a video screen. if we want to input commands to the box, we plug in a keyboard and a mouse.</p><p>lxd is a system to run containers. it knows the plug-ins for the box.</p><p>docker is a system to run containers too. so, what's the difference?</p>",
					"markdown": "## what is a container?\n\n#### *first things first..*\n\n\n\ni guess the zeroth thing is this is linux.\n\ni've used a vm or two, which can be considered containers, on windows before. vmware and virtualbox are the two main runners of these. a vm lets an os run inside an os. there are other ones like dosbox and scummvm that are like mini operating systems inside a container, the os made to serve the one main task. and then there is wsl.. windows subsystem for linux.. a linux box in windows.\n\nso, a container is a box, like a desktop computer is a box. there's stuff inside the box which has everything needed to operate and run a system (and run smaller boxes).\n\nbut.. power and adapters and peripherals and cables.. other things are still needed, to do much of anything, with this box.\n\nall these things, this system, is what we must get used to.\n\nthe insides of the box and how that works isn't very important. it's what we can do with the box, and what the box can do for us.\n\nif we want to see what a computer is doing, we have to plug in a video screen. if we want to input commands to the box, we plug in a keyboard and a mouse.\n\nlxd is a system to run containers. it knows the plug-ins for the box.\n\ndocker is a system to run containers too. so, what's the difference?\n\n"
				}
			}
		});
	...
	component_7 = new Component$8({
			props: {
				...
				description: "lxd vs docker",
				content: {
					"html": "<h2>leapt..</h2><h4><em>i didn't read the f'n manual</em></h4><p></p><p>nothing went wrong (as yet). i just dove a bit quick, i think.</p><p>of course, before writing this article i looked up a couple of informative webpages on the internet, about containers and docker and lxd. but i didn't really read them before just deciding to install ubuntu on lxd. hmm.. well, i did, and i suppose, it's meant to be!</p><p>while waiting, i saw there were containers for fedora, centos, and things like busybox and pi-hole.. there are lxd and docker containers..</p><p>i saw there is also a docker ubuntu container. now that's doubly interesting since i can't understand how to use this lxd ubuntu installation. the terminal wants a login and password. ubuntu ubuntu doesn't work. neither does my nas account. i can't really find answers..</p><p>so, ha, i'm going to delete it. then i'm going to try the docker version..</p><p>i'll note that to install the ubuntu lxd container, i got a template. lxd uses the template to build up the container. in removing it, there is the container and the image.. then i had a power outage! it's a cold winter's day..</p><p>maybe an image is like an installation dvd.. and the container is what the dvd installs into.. i'm a little confused and annoyed.. all part of the fun..</p><p>i just found out.. lxd is pronounced: lex-dee</p><p>and there's a thing called lxc. for commands.. i didn't read these instructions well at all..</p><p>ok, i'm trying docker now..</p><p>ok, this is more promising..</p><p>i am in as root in my nas ubuntu. now what?.. i'm guessing i should make a user for myself..</p><p>there is much more information going down docker street. no wonder i never heard of lxd.</p><p> </p><p>nah, i give up.. </p><p>i don't get it. i can't apt install or apt update. there is no sudo package. not even python on there. i don't get it. and i don't really care about running ubuntu on my nas anyway.</p><p>i'll try a different docker container next time. not an os. just an app or something. something small.</p><p>good night moon.. </p><p>up above the dock of 23 bay </p><p>:)</p>",
					"markdown": "## leapt..\n\n#### *i didn't read the f'n manual*\n\n\n\nnothing went wrong (as yet). i just dove a bit quick, i think.\n\nof course, before writing this article i looked up a couple of informative webpages on the internet, about containers and docker and lxd. but i didn't really read them before just deciding to install ubuntu on lxd. hmm.. well, i did, and i suppose, it's meant to be!\n\nwhile waiting, i saw there were containers for fedora, centos, and things like busybox and pi-hole.. there are lxd and docker containers..\n\ni saw there is also a docker ubuntu container. now that's doubly interesting since i can't understand how to use this lxd ubuntu installation. the terminal wants a login and password. ubuntu ubuntu doesn't work. neither does my nas account. i can't really find answers..\n\nso, ha, i'm going to delete it. then i'm going to try the docker version..\n\ni'll note that to install the ubuntu lxd container, i got a template. lxd uses the template to build up the container. in removing it, there is the container and the image.. then i had a power outage! it's a cold winter's day..\n\nmaybe an image is like an installation dvd.. and the container is what the dvd installs into.. i'm a little confused and annoyed.. all part of the fun..\n\ni just found out.. lxd is pronounced: lex-dee\n\nand there's a thing called lxc. for commands.. i didn't read these instructions well at all..\n\nok, i'm trying docker now..\n\nok, this is more promising..\n\ni am in as root in my nas ubuntu. now what?.. i'm guessing i should make a user for myself..\n\nthere is much more information going down docker street. no wonder i never heard of lxd.\n\n\n\nnah, i give up..\n\ni don't get it. i can't apt install or apt update. there is no sudo package. not even python on there. i don't get it. and i don't really care about running ubuntu on my nas anyway.\n\ni'll try a different docker container next time. not an os. just an app or something. something small.\n\ngood night moon..\n\nup above the dock of 23 bay\n\n:)\n\n"
				}
			}
		});
